...
Usage:
    ...
    python download_nsddata.py --only-rois        # download only ROI masks/atlases
"""
...
DEFAULT_OUTPUT_ROOT = Path(default_raw_data_root())
...
def download_rois():
    ...
    # Specific atlas files we need. The combined files support the standard
    # pipeline; the lh./rh. files support whole-brain seed-bank diagnostics.
    base_roi_files = [
        "nsdgeneral.nii.gz",
        "HCP_MMP1.nii.gz",
        "Kastner2015.nii.gz",
    ...
    hemi_roi_files = [
        f"{hemi}.{roi_file}"
        for hemi in ("lh", "rh")
        for roi_file in base_roi_files
    ]
    roi_files = base_roi_files + hemi_roi_files
    ...
    for sub in SUBJECTS:
        ...
        for roi_file in roi_files:
            ...
            download_s3_file(s3_path, local_path, f"  Subject {sub}: {roi_file}")
...
def download_rest_timeseries():
    ...
    for sub in SUBJECTS:
        ...
        manifest_dir = f"data/processed/subj{sub:02d}"
...
def main():
    ...
    parser.add_argument("--only-rois", action="store_true", help="Download only ROI masks and atlases")
    parser.add_argument(
        "--signed-request",
        action="store_true",
